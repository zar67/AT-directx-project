...
{
public:
	VertexConstantBuffer(Graphics& graphics, T& constantData) 
	{
	...
	VertexConstantBuffer(Graphics& graphics)
	{
		D3D11_BUFFER_DESC bufferDescription;
		bufferDescription.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
		bufferDescription.Usage = D3D11_USAGE_DYNAMIC;
	...
	{
		graphics.GetDeviceContext()->VSSetConstantBuffers(0, 1, m_pConstantBuffer.GetAddressOf());
	}
