...
std::vector<Vector> OBBCollider::GetTransformedAxes()
{
	std::vector<Vector> axis = {};

	DirectX::XMFLOAT3 axisFloat = DirectX::XMFLOAT3(1.0f, 0.0f, 0.0f);
	DirectX::XMVECTOR convertedAxis = DirectX::XMVector3TransformNormal(DirectX::XMLoadFloat3(&axisFloat), m_pTransform->GetTransformMatrixWithConstraints(m_constrainXRotation, m_constrainYRotation, m_constrainZRotation));
	DirectX::XMFLOAT3 convertedAxisFloat;
	DirectX::XMStoreFloat3(&convertedAxisFloat, convertedAxis);
	axis.push_back(Vector(convertedAxisFloat));

	axisFloat = DirectX::XMFLOAT3(0.0f, 1.0f, 0.0f);
	convertedAxis = DirectX::XMVector3TransformNormal(DirectX::XMLoadFloat3(&axisFloat), m_pTransform->GetTransformMatrixWithConstraints(m_constrainXRotation, m_constrainYRotation, m_constrainZRotation));
	convertedAxisFloat;
	DirectX::XMStoreFloat3(&convertedAxisFloat, convertedAxis);
	axis.push_back(Vector(convertedAxisFloat));

	axisFloat = DirectX::XMFLOAT3(0.0f, 0.0f, 1.0f);
	convertedAxis = DirectX::XMVector3TransformNormal(DirectX::XMLoadFloat3(&axisFloat), m_pTransform->GetTransformMatrixWithConstraints(m_constrainXRotation, m_constrainYRotation, m_constrainZRotation));
	convertedAxisFloat;
	DirectX::XMStoreFloat3(&convertedAxisFloat, convertedAxis);
	axis.push_back(Vector(convertedAxisFloat));
...
float OBBCollider::GetMinPositionOnAxis(int axisIndex)
{
	DirectX::XMFLOAT3 vertexFloat = m_minimumPosition.AsFLOAT3();
	DirectX::XMVECTOR convertedVertex = DirectX::XMVector3Transform(DirectX::XMLoadFloat3(&vertexFloat), m_pTransform->GetTransformMatrixWithConstraints(m_constrainXRotation, m_constrainYRotation, m_constrainZRotation));

	DirectX::XMFLOAT3 convertexVertexFloat;
	DirectX::XMStoreFloat3(&convertexVertexFloat, convertedVertex);

	Vector transformed = Vector(convertexVertexFloat);
...
float OBBCollider::GetMaxPositionOnAxis(int axisIndex)
{
	DirectX::XMFLOAT3 vertexFloat = m_maximumPosition.AsFLOAT3();
	DirectX::XMVECTOR convertedVertex = DirectX::XMVector3Transform(DirectX::XMLoadFloat3(&vertexFloat), m_pTransform->GetTransformMatrixWithConstraints(m_constrainXRotation, m_constrainYRotation, m_constrainZRotation));
